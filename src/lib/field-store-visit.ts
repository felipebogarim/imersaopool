...
export function isFieldStoreVisit(md: string): boolean {
  ...
  return (
    ...
    /(^|\n)\s*[-*]?\s*schema_version\s*:\s*1\.[1-9]/i.test(t)
  );
}
...
export function parseFieldStoreVisit(md: string): {
  ...
  const detected_version = meta["schema_version"] || "1.0";
  ...
  let inMeta = false;
    ...
    const flush = () => {
      if (!cur) return;
      const chapters_config = detected_version === "1.1" 
      ...
      const known = (chapters_config as any)[cur.ordem];
      chapters.push({
        ordem: cur.ordem,
  ...
  const required = detected_version === "1.1" ? [1, 7] : [0, 1, 8];

  for (const req of required) {
    if (!found.has(req)) {
      const label = (chapters_config as any)[req].titulo;
      errors.push(`O Capítulo ${String(req).padStart(2, "0")} — ${label} não foi encontrado no documento.`);
    }
  }

  chapters.sort((a, b) => a.ordem - b.ordem);
  if (errors.length) return { doc: null, errors, detected_version };
  return { doc: { meta, chapters }, errors: [], detected_version };
}
