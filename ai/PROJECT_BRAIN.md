...
- **BI Diretor / Reps:** Representative follow-up fields are persisted in `director_rep_notes`, one row per representative and scoped to the active company. Authenticated company users may read them; only the signed-in master account `felipe@poolbranding.com.br` may insert or update them, enforced by UI visibility and RLS using the signed JWT email. The UI keeps the representative list available and disables editing while the table is absent; persistence requires applying migration `20260924120000` (the current CLI account returned 403 when checking the linked project).
