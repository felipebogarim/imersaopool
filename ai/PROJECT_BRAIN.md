...
- **Solicitações Internas (Fases 1–11 de 12 implementadas; só falta a Fase 12 — aplicar de verdade. Nada rodou contra um banco/conta real ainda; ver `internal-tickets-setup.md` na raiz do repo para o passo a passo manual pendente):** módulo de tickets do Comercial pra outros setores, schema `internal_ticket_*` (7 migrations, `20260923080000` a `130000` — ordem e conteúdo listados em `internal-tickets-setup.md`), setores/pessoas/categorias globais, tickets por `company_id`. Papéis novos em `app_role`: `gestor_comercial` (não reaproveita `gestor`) e `diretoria` (read-only) — nav_key nova exige seed explícito em `role_permissions`. Camada de e-mail desacoplada (`EmailProvider`) usa **Resend**; `INTERNAL_TICKETS_EMAIL_MODE=mock` troca pro `FakeEmailProvider` em memória sem credencial (Fase 7, `provider-factory.server.ts`). UI completa: `/admin/solicitacoes-internas` (CRUD), `/solicitacoes/novo` (cria e já envia), `/solicitacoes` (lista), `/solicitacoes/$ticketId` (Detalhe: timeline unificada, mudança de status via `updateInternalTicketStatus` — canTransition estrito, diferente do canReach das ações por e-mail —, registro de interação manual, upload de anexo pro bucket privado `internal-ticket-attachments`), `/solicitacoes/dashboard` (KPIs/SLA/volumes, tudo em `dashboard-metrics.ts`, puro e testado). Webhook `/api/public/internal-tickets/resend-webhook` (Svix oficial; evento inbound `email.received` assumido, não confirmado contra chamada real — ver setup doc). **Revisão de segurança (Fase 10) encontrou e corrigiu 3 problemas antes de fechar**: (1) as policies de INSERT de `internal_ticket_events`/`internal_ticket_messages` só checavam acesso ao ticket, não o `origin` — qualquer usuário com acesso podia forjar um evento com origin='email'/'sistema' fingindo uma resposta real; migration `130000` restringe o caminho autenticado a origin='comercial' (eventos) e origin IN ('manual_presencial','manual_telefone') (mensagens), sempre com `author_user_id/uploaded_by = auth.uid()`; (2) isso quebrou um insert legítimo em `sendInternalTicket` que gravava origin='sistema' pelo client RLS-scoped — corrigido pra usar `supabaseAdmin`; (3) bucket de anexos sem `file_size_limit` no Storage (só havia checagem em JS, contornável) — setado 20MB no bucket. Risco residual aceito: RLS não impede um `PATCH` direto mudando `status` pra um valor fora do grafo de `canTransition` — a validação da máquina de estados é só na camada de aplicação, não em trigger SQL (documentado em `internal-tickets-setup.md`). Commit/push feitos em `orca-dev` (`e2325128`). Migration `140000` (add-on posterior) semeia 7 categorias iniciais — sem ela o campo obrigatório Categoria em Novo Ticket ficava sem nenhuma opção (só setores vinham pré-cadastrados); nomes vieram literalmente da lista de exemplos do briefing original, não inventados.
