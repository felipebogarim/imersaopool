...
- Git: `main` is the only operational/canonical branch shared by Orca, GitHub and Lovable; `orca-dev` is legacy only. Sync `origin/main` before work and re-check the remote before push; task branches only for concrete isolation; no force push, destructive reset, or rebase/amend of published history.
...
- Preserve published git history on `main`, the Lovable-connected branch.
