...
BEGIN
  ...
  INSERT INTO public.rep_performance_uploads(
    ...
    'performance_v4_deterministic_excel',
  ...
  FOR _row IN SELECT * FROM jsonb_array_elements(_rows) LOOP
    ...
    FOREACH _fam IN ARRAY _familias LOOP
      ...
      IF _real_val IS NULL AND _fam_target > 0 AND _pct_val IS NOT NULL THEN
        _real_val := _fam_target * _pct_val;
      END IF;
      IF _real_val IS NULL AND _fam_target > 0 AND _factor IS NOT NULL THEN
      ...
      END IF;
      IF _real_val <> 0 OR _fam_target > 0 THEN
    ...
    VALUES (_row_id, _new_upload_id, COALESCE(NULLIF(_total_meta,0), _row_total_meta_input), _row_metas, _realizado);
  END LOOP;
  ...
  RETURN jsonb_build_object(
    ...
    'calculation_version', 'performance_v4_deterministic_excel'
  );
END $function$;
