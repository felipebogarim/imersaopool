  ...
  LOOP
    ...
    BEGIN
      ...
      FOREACH _f IN ARRAY _familias LOOP
        ...
        IF _cat <> '' AND _meta_val IS NOT NULL AND _meta_val > 0 AND _real_val IS NOT NULL THEN
      ...
      SET familia_pct = _fpct,
          total_pct = CASE
            WHEN _row.stored_total_pct IS NOT NULL THEN _row.stored_total_pct
    ...
    IF EXISTS (SELECT 1 FROM jsonb_each_text(COALESCE(_row.realizado, '{}'::jsonb))) THEN
